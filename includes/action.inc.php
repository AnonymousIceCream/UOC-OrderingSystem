...
mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);
    ...
    if(isset($_POST['action']) && isset($_POST['action']) == 'order')
    {
        ...
        $data = '';
         ...
         $sql1 = "SELECT deliveryFee FROM tbl_deliveryFee WHERE postalCode = $postalcode";
            ...
            $count1 = mysqli_num_rows($res1);
                if($count1 > 0) {
                    ...
                    echo '<div>may mali</div>';
                }

         $grand_total = $deliveryfee + $cart_total;

        //Input User Information to DB
        $sql = 'INSERT INTO tbl_orders (name, email, phonenumber, address, city, postalcode, region, order_items, paymentmode, order_date, grand_total, order_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);';
        ...
        if(!mysqli_stmt_prepare($stmt, $sql))
        {
         ...
         mysqli_stmt_bind_param($stmt, 'ssssssssssss', $name, $email, $phone, $address, $city, $postalcode, $region, $products, $pmode, $order_date, $grand_total, $order_status);
        ...
        // Thankyou Message
          $data .= '<div class="text-center">
                            ...
                            <h4>Grand Total with delivery Fee : ₱' . number_format($grand_total,2) . '</h4>
